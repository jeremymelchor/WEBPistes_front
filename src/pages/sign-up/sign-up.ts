...
export class SignUp {
    ...
    signUp(): void {
        if (this.socketIoService.socket == undefined) {
            console.log("socket undefined");
            this.socketIoService.getTeamColor().subscribe(
                res => {
                    console.log("success", res)
                    if (res == 'KO') this.socketIoService.closeSocket();
                    else {
                        this.teamColor == undefined ?
                            this.socketIoService.teamColor = "#f4f4f4" : this.socketIoService.teamColor = this.teamColor;
            ...
            this.socketIoService.signUp(this.teamName);
        }
